...
class ASR(sb.Brain):
    def compute_forward(self, batch, stage):
        ...
        # forward modules
        src = self.modules.lin_in(feats)
        src, attn_mask, key_padding_mask = pad_and_creeate_mask(src,
                                                                wav_lens,
                                                                self.hparams.look_back,
...
def pad_and_creeate_mask(batch, wav_len, look_back, look_ahead, chunk_length):
    b, t_0, e = batch.shape
    device = batch.device
    # pad batch with zeros if the last chunk has fewer frames than chunk_length
    padding_amount = chunk_length - t_0 % chunk_length
    batch = F.pad(batch, (0, 0, 0, padding_amount), 'constant', 0)

    # create src mask for restricted attention v_(n-lb), v_(n-lb+1)....v_(n)..v_(n+la)
    b, t_1, e = batch.shape
    look_ahead = torch.tril(torch.ones(t_1, t_1), diagonal=look_ahead)
    look_back = torch.triu(torch.ones(t_1, t_1), diagonal=-look_back)
    restricted_attn_mask = (look_ahead * look_back).to(device)

    # create key padding with relative sequence lengths
    abs_len = torch.round(wav_len * t_0)
    src_key_padding_mask = length_to_mask(abs_len)
    src_key_padding_mask = F.pad(src_key_padding_mask, (0, padding_amount), 'constant', 0)

    # src mask for dilated frames
    L = t_1 // chunk_length
    src_mask_for_chunks = []
    for i in range(L):
        src_mask_for_i_th_chunk = torch.zeros((chunk_length, L), device=device)
        src_mask_for_i_th_chunk[:, :i] = 1
        src_mask_for_chunks.append(src_mask_for_i_th_chunk)
    src_mask_for_chunks = torch.cat(src_mask_for_chunks)

    # key paddings for dilated frames
    key_padding_mask_for_chunks = torch.zeros((b, L), device=device)
    for i, l in enumerate(wav_len):
        wav_len_to_chunk_len = round(l.item() * t_0) // chunk_length
        key_padding_mask_for_chunks[i, :wav_len_to_chunk_len] = 1

    # concatenate masks for dilation frames to the original mask and convert it to boolean
    # we have used the value of 1 as allow and value of zero as mask hwoever torch uses False or True to mask a position
    # so we need to convert it to boolean and negate the values
    restricted_attn_mask = ~(torch.cat((restricted_attn_mask, src_mask_for_chunks), dim=1)).bool()
    src_key_padding_mask = ~(torch.cat((src_key_padding_mask, key_padding_mask_for_chunks), dim=1)).bool()
    return batch, restricted_attn_mask, src_key_padding_mask
